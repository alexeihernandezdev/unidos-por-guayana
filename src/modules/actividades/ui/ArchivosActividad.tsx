...
import type { ArchivoVista } from "@/shared/actividades";
import {
  ACCEPT_ADJUNTO,
  ACCEPT_IMAGEN,
  formatearTamano,
...
import { eliminarArchivoAction } from "@/app/(admin)/panel/actividades/actions";
...
// Gestiona la imagen principal y los documentos de una actividad DIRECTO al almacenamiento
// (feature 033): pide una URL firmada al servidor, hace PUT del binario y confirma los
// metadatos. Solo la usa el ADMIN dueño en el detalle, en cualquier estado.
export function ArchivosActividad({
  ...
  async function subir(file: File, tipo: TipoArchivo): Promise<void> {
    ...
    setOcupado(tipo);
    try {
      const resultado = await subirArchivoDirecto(actividadId, file, tipo);
      if (!resultado.ok) {
      ...
      if (tipo === TipoArchivoActividad.PRINCIPAL) {
        if (principal?.url?.startsWith("blob:")) {
          URL.revokeObjectURL(principal.url);
        }
        setPrincipal(vista);
      } else {
  ...
  async function eliminar(archivo: ArchivoVista): Promise<void> {
    ...
    setOcupado("eliminar");
    try {
      const resultado = await eliminarArchivoAction(archivo.id, actividadId);
      if (!resultado.ok) {
        setError(resultado.error);
        return;
      }
      if (archivo.url?.startsWith("blob:")) URL.revokeObjectURL(archivo.url);
      if (archivo.tipo === TipoArchivoActividad.PRINCIPAL) {
        setPrincipal(null);
      } else {
  ...
  const subiendoPrincipal = ocupado === TipoArchivoActividad.PRINCIPAL;
  const subiendoAdjunto = ocupado === TipoArchivoActividad.ADJUNTO;
  const adjuntosLleno = adjuntos.length >= MAX_ADJUNTOS;

  return (
    <fieldset className="flex flex-col gap-6">
      <legend className="sr-only">Imagen y documentación</legend>
      ...
      <div className="flex flex-col gap-2">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (file) void subir(file, TipoArchivoActividad.PRINCIPAL);
            e.target.value = "";
          }}
        />
          ...
          onChange={(e) => {
            ...
            void (async () => {
              for (const file of archivos) {
                await subir(file, TipoArchivoActividad.ADJUNTO);
              }
            })();
            e.target.value = "";
          }}
