...
import type { ArchivosVista } from "@/shared/actividades";
import { formatearTamano, iconoDeContentType } from "@/modules/archivos/ui/archivos";

type Props = {
  archivos: ArchivosVista;
  /** Título de la sección. Por defecto "Imagen y documentación". */
  titulo?: string;
};

// Muestra la imagen principal y los adjuntos de una actividad en solo lectura (detalle del
// colaborador). Si el almacenamiento no está disponible, degrada a los nombres sin enlace
// (feature 033). Las URLs son públicas y permanentes.
export function ArchivosActividadVista({
  archivos,
  titulo = "Imagen y documentación",
}: Props) {
        ...
        <div className="flex flex-col gap-2">
          ...
          <ul className="divide-y divide-border rounded-lg border border-border">
            {adjuntos.map((archivo) => {
              ...
              return (
                <li key={archivo.id} className="flex items-center gap-3 px-4 py-3">
                  <Icono
                    strokeWidth={1.5}
