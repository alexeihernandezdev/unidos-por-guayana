...
- [ ] Repasar el módulo `ayudas` (005): `domain/EstadoAyuda.ts`, `domain/Ayuda.ts`, `domain/reglas.ts`,
...
- [ ] Añadir a `schema.prisma`: enum `TipoActividad { ENVIO JORNADA EVENTO_SOCIAL }` y el campo
      `tipo TipoActividad @default(ENVIO)` en `model Ayuda`.
- [ ] Migración `add_tipo_actividad`: crear el enum, añadir la columna con default `ENVIO` (**backfill**
...
- [ ] `TipoActividad` (const-object + unión, mismos valores que Prisma), en el patrón de `EstadoAyuda`.
- [ ] Añadir `tipo: TipoActividad` a la entidad `Ayuda` y a `NuevaAyuda`. **No** añadirlo a
      `CambiosAyuda` (inmutable tras el alta).
- [ ] En `reglas.ts`: validar `tipo` como valor válido de `TipoActividad` al crear.
- [ ] Exportar `TipoActividad` desde `domain/index.ts`.

## 3. Aplicación (`src/modules/ayudas/application`)

- [ ] `crearAyuda`: aceptar, validar y persistir `tipo`; resto del flujo intacto.
- [ ] `listarAyudas`: extender el filtro a `{ estado?; tipo? }`, combinables.
- [ ] `obtenerAyuda` / `editarCabecera`: sin cambios de lógica (el `tipo` viaja en la entidad y no está
      en `CambiosAyuda`).
- [ ] Actualizar los dobles en `fakes.ts` para contemplar `tipo`.
- [ ] Mantener la capa pura (solo `domain` + contrato de `RecursoRepository`).

## 4. Infraestructura

- [ ] `PrismaAyudaRepository`: mapear `tipo` en lectura y escritura; aplicar filtro por `tipo` en
      `listar` (junto al de `estado`), sin casts de enums a mano.

## 5. Presentación (solo `ADMIN`)

- [ ] Mapa de presentación por tipo en `ayudas/ui` (p. ej. `tipos.ts`): nombre singular + etiqueta de
      badge por cada `TipoActividad`. Único punto de verdad de los copys por tipo.
- [ ] **Alta** `/(admin)/panel/ayudas/nueva`: selección del `tipo` y **renombrado** de título/botón
      ("Crear envío" / "Crear jornada" / "Crear evento social"); `AyudaForm` envía el `tipo`.
- [ ] **Listado** `/(admin)/panel/ayudas`: mostrar el `tipo` en `AyudasTabla` y añadir **filtro por
      tipo** junto al de estado.
- [ ] **Detalle** `/(admin)/panel/ayudas/[id]`: presentar la cabecera con el vocabulario del tipo.
- [ ] Server action de crear: aceptar y validar `tipo` con `zod` (enum de tres valores); las de editar
      cabecera **no** aceptan `tipo`. Mantener `requireRol(ADMIN)` y `revalidatePath`.
- [ ] Sin em-dash (`—`) / en-dash (`–`) en los textos por tipo; fechas con Luxon (`es-VE`, `DD/MM/AAAA`).

## 6. Composición (wiring)

- [ ] Sin nuevos repos/casos de uso: solo extender firmas. Respetar los límites de capas (`app`/`ui` no
      importan `infrastructure`/`lib` directamente).

## 7. Tests (Vitest)

- [ ] Reglas de dominio: `tipo` válido / inválido.
- [ ] `crearAyuda`: crea con cada `tipo`; rechaza `tipo` inválido.
- [ ] `listarAyudas`: filtra por `tipo`, por `estado` y por ambos combinados.
- [ ] `editarCabecera`: el `tipo` no puede cambiar.
- [ ] Confirmar que **toda la suite de 005 sigue en verde** tras el cambio aditivo.
...
- [ ] `pnpm test` en verde (incluida la suite de 005).
...
- [ ] Mover `018 · Tipos de actividad en Ayuda` a **Hecho ✅** en `constitution/roadmap.md` (dejando
