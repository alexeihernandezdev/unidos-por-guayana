...
13. **016 · Perfil de administrador y centro de acopio** — `PerfilAdmin` con datos ampliados (`nombreCuenta`, `estado`, `parroquia`, `telefono`, `correo`, `documento` con `tipoDocumento` ∈ `JURIDICO` | `NATURAL`); el `ADMIN` funciona como centro de acopio y puede tener uno o varios `PuntoAcopio`. _Depende de 015; conecta con 011._
