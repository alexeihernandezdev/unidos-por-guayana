...
20. **014 · Donaciones monetarias externas** — Catálogo administrable de **medios de donación** (`MedioDonacion`: cuenta bancaria, Pago Móvil, PayPal, Zelle, Binance, efectivo…) que el `ADMIN` publica u oculta en `/panel/donaciones` y el público ve en `/transparencia`, y **registro manual de montos ya recibidos por fuera** (`registrarAporteExterno`): crea un `Aporte` `MONETARIO` que nace en `RECIBIDO`, con o sin colaborador y con o sin actividad ("caja general"). La app **no procesa pagos**: solo muestra medios y anota lo recibido. Módulo `src/modules/donaciones` (Clean + Screaming); enmienda 006 (`Aporte.colaboradorId` y `ayudaId` opcionales, `registradoPorId`/`medioDonacionId`/`moneda`/`referencia`). _Enmienda 006; alimenta 009._

## Siguiente 🔜

_Lo próximo a abordar. Idealmente una sola feature "en curso" a la vez._

20. **010 · Seguimiento del envío** — Historial de trazabilidad (`SeguimientoEvento`): transiciones de estado y evidencia de entrega.
...
21. **010 · Seguimiento del envío** — Historial de trazabilidad (`SeguimientoEvento`): transiciones de estado y evidencia de entrega.

**Módulos de apoyo**

22. **011 · Puntos de acopio** — Centros físicos de entrega (dirección, horarios, qué reciben), **cada uno perteneciente a un `ADMIN`** (ver 016).
23. **024 · Actividad: renombre y ciclo de vida por tipo** — `Ayuda` se renombra a `Actividad` en todas las capas (modelo, módulo, rutas); `ENVIO` conserva `RECOLECTANDO → LISTO → EN_TRANSITO → ENTREGADO`, `JORNADA`/`EVENTO_SOCIAL` pasan a `RECOLECTANDO → LISTA → EN_CURSO → REALIZADA`; se añade `horaFin` opcional y `puntoAcopioId` opcional (del mismo admin dueño). _Enmienda 005, 008, 009, 012, 018, 022, 023. Depende de 018 y 022._
24. **025 · Afiliación a centros de acopio y categorías de aporte** — El `COLABORADOR` declara (obligatorio, en el registro) al menos una `CategoriaRecurso` que podría aportar, y se afilia (opcional, omitible) a uno o varios `ADMIN`. El admin ve "su red" en `/panel/red` (con contacto y filtro por categoría), puede remover afiliados (sin bloquear re-afiliación), y ve un conteo de afiliados aptos al elegir el recurso de una meta al crear una Actividad. Puramente informativa: no restringe a qué Actividad puede aportar un colaborador. _Enmienda 002 y 017. Depende de 002, 013, 016, 020 y 024._
25. **012 · Notificaciones** — Avisos a colaboradores sobre envíos que necesitan recursos o metas cumplidas. **Actualizada:** el disparador `NUEVA_AYUDA`/`NUEVA_ACTIVIDAD` notifica a la **red apta** del admin dueño (afiliados verificados cuya categoría coincide con la de algún recurso de la Actividad, ver 025), no a "todos los colaboradores verificados" como se especificó originalmente. Depende de 025 (se implementa después, ya con esta lógica).
